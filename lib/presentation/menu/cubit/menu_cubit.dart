...
class MenuCubit extends Cubit<MenuState> {
  ...
  Future<void> selectCategory(String categoryId) async {
    ...
    if (currentState is MenuLoaded) {
      emit(currentState.copyWith(isLoading: true, selectedCategoryId: categoryId));
      try {
  ...
  Future<void> searchMenu(String query) async {
    ...
    if (currentState is MenuLoaded) {
      ...
      try {
        final items = await getMenuUseCase.getMenuItems(
          categoryId: query.trim().isNotEmpty ? 'All' : currentState.selectedCategoryId,
          searchQuery: query,
        );
        emit(currentState.copyWith(
          menuItems: items,
          selectedCategoryId: query.trim().isNotEmpty ? 'All' : currentState.selectedCategoryId,
          searchQuery: query,
          isLoading: false,
        ));
      } catch (e) {
